...
parser.add_option(
    ...
    type="str",
    default=1,
    help="Index of x column, one-based (0 for position index), or col name if using FITS data",
)
parser.add_option(
    "-y",
    dest="y",
    type="str",
    default=2,
    help="Index of y column, or col name if using FITS data",
)
parser.add_option(
    "-e", dest="e", type="str", default=None, help="Index of error bar column, if any"
)
parser.add_option(
    "-c", dest="c", type="str", default=None, help="Index of colour column, if any"
)
parser.add_option(
    "-z",
    "--xy",
    dest="xy",
    action="append",
    help="Plot pairs of columns specified as x,y",
)
parser.add_option(
    "-T",
    "--transpose",
    ...
    default=False,
    action="store_true",
    help="Transpose the data just after loading it.  Only applies to text, not fits.",
)
...
parser.add_option(
    ...
    type="int",
    default=0,
    help="Skip the first SKIP rows of the files",
)
parser.add_option(
    "-E", "--every", dest="every", type="int", default=1, help="Plot every n'th point"
)
parser.add_option(
    "-m",
    "--math",
    dest="math",
    action="append",
    help="Apply modifications to the x and y here, for example, plot file -m 'x=x*2'  -m 'y=log(y)' ",
)
...
parser.add_option(
    ...
    default="-",
    const=".",
    help="Plot with lines rather than points",
)
parser.add_option(
    "-d",
    "--dots",
    ...
    default="-",
    const=",",
    help="Plot with dots rather than points",
)
parser.add_option(
    "-l",
    "--linespoints",
    ...
    default="-",
    const=".-",
    help="Plot with lines and points",
)

parser.add_option("-t", "--title", dest="title", default="Plot", help="Plot title")
parser.add_option(
    "--xt", "--xtitle", "--xlabel", dest="xtitle", default="x", help="Title of x axis"
)
parser.add_option(
    "--yt", "--ytitle", "--ylabel", dest="ytitle", default="y", help="Title of y axis"
)
parser.add_option(
    "--ct",
    "--ctitle",
...
parser.add_option(
    ...
    dest="tex",
    default=False,
    help="Use latex for the labels",
)


parser.add_option(
    "--xmin", dest="xmin", type="float", default=None, help="x range minimum"
)
parser.add_option(
    "--xmax", dest="xmax", type="float", default=None, help="x range minimum"
)
parser.add_option(
    "--ymin", dest="ymin", type="float", default=None, help="y range minimum"
)
parser.add_option(
    "--ymax", dest="ymax", type="float", default=None, help="y range minimum"
)
...
parser.add_option(
    ...
    action="store_true",
    default=False,
    help="Show plot regardless of saving",
)
...
parser.add_option(
    ...
    dest="spectrum",
    default=False,
    help="Plot the power spectrum of the y axis",
)
parser.add_option(
    "-a",
    "--moving-average",
    ...
    type="int",
    default=0,
    help="Generate a moving average plot (both axes)",
)
parser.add_option(
    "-f",
    "--fit-poly",
    ...
    type="int",
    default=0,
    help="Fit a polynomial of the specified order and plot it",
)
...
parser.add_option(
    ...
    dest="histogram",
    default=False,
    help="Plot a histogram of the y column, and ignore the x column",
)
parser.add_option(
    "-S",
    "--step",
    ...
    dest="hist_step",
    default=False,
    help="Plot unfilled (step-style) when doing histograms",
)
parser.add_option(
    "-b",
    "--bins",
...
parser.add_option(
    ...
    action="store",
    default="",
    help="Read the input, find lines with the specified prefix, and plot them",
)
...
parser.add_option(
    ...
    dest="force_fits",
    default=False,
    help="Force a file to be read as a FITS file regardless of its suffix",
)
parser.add_option(
    "-j",
    "--extension",
    ...
    dest="extension",
    default="1",
    help="FITS extension number to read.",
)
parser.add_option(
    "-N",
    "--nofits",
    ...
    dest="use_fits",
    default=True,
    help="Do not plot the file as FITS, regardless of its suffix",
)
...
parser.add_option(
    ...
    dest="nolegend",
    default=False,
    help="Do not include a legend on the plot",
)
parser.add_option(
    "-P",
    "--legend-position",
    ...
    dest="legend_pos",
    default="upper left",
    help="Where to put the legend - 'upper left', 'center', etc.'",
)
...
parser.add_option(
    ...
    dest="central_cross",
    default="",
    help="Add a cross marker at the single specified point, e.g. -C 1.4,5.33",
)
...
def make_hist_plotter(opt):
    ...
    if opt.log:
        xlog = True
        ylog = True
    density = 1 if opt.norm else False
    hist_range = None
    histtype = "bar"
    if opt.hist_step:
        histtype = "step"
    ...
    def hist_plotter(x, y, *args, **kwargs):
        if ylog:
            y = log10(y)
        hist(
            y, bins=bins, log=xlog, density=density, range=hist_range, histtype=histtype
        )
...
def plot_col(
    data, i, j, fmt, filename, plotter, extra_math, e, c, averaging, poly, tex
):
    """
...
def load_text(filename, opt):
    if opt.prefix:
        ...
        data = genfromtxt(
            filename, unpack=True, skip_header=opt.skip, invalid_raise=False
        )
    return data
    ...
    for filename in files:
        if filename == "-":
            ...
            if data.ndim == 2:
                ...
                data = data[:: opt.every]
        elif (filename.endswith(".hdf5") or opt.force_fits) and opt.use_fits:
            import h5py
            ...
            with h5py.File(filename, "r") as f:
                g = f[ext]
                d = {col: g[col][:][:: opt.every] for col in g.keys()}
                max_len = np.max([len(d) for d in d.values()])
                d = {col: val for col, val in d.items() if len(val) == max_len}
                dt = np.dtype(
                    {
                        "names": list(d.keys()),
                        "formats": [col.dtype for col in d.values()],
                    }
                )
                data = np.empty(max_len, dtype=dt)
...
if (not options.use_fits) and options.force_fits:
    stderr.write(
        "You have specified both to use FITS mode and not to use it.  It will not be used.\n"
    )
import matplotlib
